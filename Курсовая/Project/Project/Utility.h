...
class Utility : public SystemSoftware
{
public:
	Utility() : SystemSoftware() {
		_ensure = "Неизвестно";
	}
	Utility(std::string name, std::string version, int cost, int size, std::string specialization) :
		SystemSoftware(name, version, cost, size, specialization) {
		_ensure = "Неизвестно";
	}
	Utility(std::string name, std::string version, int cost, int size, std::string specialization, std::string ensure) :
		SystemSoftware(name, version, cost, size, specialization) {
		_ensure = ensure;
	}
	std::string GetUtilEnsure() {
		return _ensure;
	}
	...
	void Print(DataTable^ table) override {
		...
		x = GetCost() + "Рублей";
		y = GetSize() + "Мб";
		txt = "Специализация: " + gcnew String(GetSystSpecialization().c_str()) + ";Обеспечение работы: " + gcnew String(GetUtilEnsure().c_str());
	...
	void SavingFile() override {
		...
		str += gcnew String(GetUtilEnsure().c_str());
		StreamWriter^ file = gcnew StreamWriter("Utility.txt", true);
		file->WriteLine(str);
		file->Close();
	}
